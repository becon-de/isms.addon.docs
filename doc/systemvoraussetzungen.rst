...
* Für Upload und Import des Grundschutzkompendiums sollte die maximale Uploadgröße und die PHP-Execution-Time mindestens folgende Größe haben:
    ...
    * ``max_input_time = 300``
* Der variable Report ``IT-Grundschutz-Check`` benutzt eine Datenbankfunktion, die von MySQL 5.6 und 5.7 noch nicht unterstützt wird. Für eine volleständige Funktionalität sollte MariaDB >=10.1 oder MySQL >=8.0 eingesetzt werden (s. `Known issues </de/latest/installation.html#known-issues>`_).